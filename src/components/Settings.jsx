...
function Settings() {
  ...
  const { user, setUser, genreList, setGenreList, purchased, setFirstName, setLastName } = useStoreContext();
  ...
  const [lName, setlName] = useState(nameArray.slice(1).join(" ") || "");
  ...
  async function changeName(event) {
    ...
    try {
      ...
      // No need to reload currentUser, just update context directly:
      setUser({
        ...user,
        displayName: newDisplayName,
      });

      // Update firstName and lastName in context immediately
      setFirstName(fName);
      setLastName(lName);
  ...
  async function changePassword(event) {
    ...
    try {
      const credential = EmailAuthProvider.credential(currentUser.email, oldPass.current.value);
  ...
  return (
    <div>
      <div className="hero">
        ...
        <button
          className="settings-back" type="submit" onClick={() => navigate(`/movies/genre/${genreList[0]?.id}`)}
        >Back</button>
        <div className="register-box settings-box">
          <div className="register-item">
            ...
            {!isEmailPasswordUser && (
              <p className="settings-text">
                You signed in using a third-party provider (e.g. Google), so your password
          ...
          <div className="register-item">
            ...
            <button className="settings-genre" onClick={() => updateGenres()}>
              Save Changes
            </button>
          </div>
        </div>
